...
from ..const import URL_BASE, COUNTRIES_VISITED_CARDS
...
class CountriesVisitedCardRegistration:
    ...
    async def async_register_countries_visited_path(self):
        ...
        self.hass.http.register_static_path(
            URL_BASE,
            self.hass.config.path("custom_components/countries_visited/frontend"),
            cache_headers=False,
        )
    ...
    async def async_wait_for_lovelace_resources(self) -> None:
        async def check_lovelace_resources_loaded(now):
            if self.hass.data["lovelace"]["resources"].loaded:
                ...
                _LOGGER.debug(
                    "Unable to install countries_visited card resources because Lovelace resources not yet loaded.  Trying again in 5 seconds."
                )
    ...
    async def async_remove_gzip_files(self):
        path = self.hass.config.path("custom_components/countries_visited/frontend")
        gzip_files = [
